...
if [ -z $NAME_ORIGIN ]; then
    git remote add origin "$URL_MODIFIED"
 else
    git remote add "$NAME_ORIGIN" "$URL_MODIFIED"
fi

# validation success run of add remote repository
if [ $? -eq 0 ]; then
    echo "✅ - Success! - Rrepository add remote successfully."
else
    echo "❌ - Error: There was a problem during add remote."
    exit 1
fi